...
async function initDb() {
  ...
  [
    ...
    // NOTE: Only products that exist in the live backend catalog are seeded here, so any
    // sale made offline can sync. The full catalog (and anything added in the admin) is
    // pulled from the server via /v1/sync/pull. Do NOT add demo products that aren't on the
    // server — bills for them fail to sync (variant foreign-key / insufficient-stock errors).
  ...
  if (cashierCount === 0) {
    const insertCashier = db.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `);
    // 'admin' uses the real backend user id so that even a first-run OFFLINE bill
    // carries a valid cashier id and syncs once online. (ravi/priya are local-only
    // demo cashiers — they are not backend users, so their bills won't sync.)
    [
      ['2c088406-dac3-4a75-a030-1ae2a672f6d5', 'admin', 'System Admin', '1234'],
...
function createWindow() {
  mainWindow = new BrowserWindow({
    width: 1280,
    height: 800,
    title: "Shubhraj Mini Mart POS - Desktop Terminal",
